/* Article-page enhancements: the meta-description blurb under the title,
 * collapsible math callouts/admonitions/code blocks, code syntax
 * highlighting, and the footnote sidenote/card system. See callouts.css,
 * code.css and footnotes.css for the matching styles. */
document.addEventListener('DOMContentLoaded', function () {
  ...
  function enhanceArticleDescription() {
    ...
    // Current exports render the description server-side (see
    // my/org-html-description-block); only typeset math on it.
    var rendered = document.querySelector('#content > .article-desc');
    if (rendered) {
      typesetDescriptionMath(rendered, rendered.textContent);
      return;
    }

    // Fallback for pages exported before that: build it from the meta tag.
  ...
  // Minimal Org inline renderer for the description: [[target][text]]
  // links plus ~code~ / =verbatim= spans (both are <code> in Org's HTML
  // export). The meta content is raw Org source, so anything else stays
  // literal text.
  function renderOrgLinks(text, container) {
    ...
    while ((match = re.exec(text))) {
      // Org only opens a marker after whitespace/opening punctuation and
      // closes it before whitespace/closing punctuation, so "a=b=c" is
      // not verbatim.
      if (match[3]) {
        var before = text.charAt(match.index - 1);
        var after = text.charAt(re.lastIndex);
      ...
      if (match[3]) {
        ...
        a.href = match[1];
        renderOrgLinks(match[2] || match[1], a);
  ...
  function enhanceProofs() {
    ...
    document.querySelectorAll('div.proof').forEach(function (block) {
      // Avoid adding duplicate controls if the initializer runs more than once.
      if (block.querySelector(':scope > .proof-toggle')) return;
      ...
      var contentId = 'proof-content-' + proofNumber;

      while (document.getElementById(contentId)) {
        proofNumber += 1;
        contentId = 'proof-content-' + proofNumber;
      }
      ...
      contentInner.className = 'proof-content-inner';
      ...
      // Proofs are often skippable, so they start collapsed — same
      // convention as the Solution/Example math callouts.
      setCollapsed(true);
