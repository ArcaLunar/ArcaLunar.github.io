...
struct Decimal {
    ...
    Decimal operator/(const Decimal &d) const { return d == 0.0 ? 2e18 : x / d.x; }
    Decimal operator/=(const Decimal &d) {
    ...
    friend ostream &operator<<(ostream &out, const Decimal &d) { return (out << d.x); }
...
namespace Geo2D {
    ...
    /* #region 平面几何相交状态 */
    enum class STATUS { Parallel, OnSameLineOverlap, OnSameLineDisjoint, SameLine, IntersectionSeg, IntersectionLine };
    inline ostream &operator<<(ostream &out, const STATUS &s) {
        switch (s) {
        case STATUS::Parallel: out << "(Lines or Segs) Parallel"; break;
        case STATUS::OnSameLineDisjoint: out << "(Segs) On same line, disjoint"; break;
        ...
        case STATUS::OnSameLineOverlap: out << "(Segs) Overlap on same line"; break;
        default: break;
        }
        return out;
    }
...
#define Point Vector
    struct Vector {
        ...
        friend ostream &operator<<(ostream &out, const Vector &a) { return (out << "(" << a.x << ", " << a.y << ")"); }
        friend istream &operator>>(istream &in, Vector &a) { return (in >> a.x >> a.y); }
        ...
        // 与 v 的夹角
        Decimal AngleBetween(const Vector &v) { return (dot(v) / Scale() / v.Scale()).acos(); }
        // 点之间的距离
        Decimal Distance(const Point &p) const { return (*this - p).Scale(); }

        // 绕点旋转
        Point Rotate(const Point &O, const Decimal &ang) { return {O.x + (x - O.x) * ang.cos() - (y - O.y) * ang.sin(), O.y + (y - O.y) * ang.cos() - (x - O.x) * ang.sin()}; }
        // 关于点 p 的对称点
        Point SymmetricPoint(const Point &p) const { return p * 2 - *this; }
        // 投影到 v 上的投影向量
        Vector ProjectVector(const Vector &v) const { return v.Unit() * dot(v) / v.Scale(); }
        // 关于 v 的对称向量
        Vector SymmetricVector(const Vector &v) const { return ProjectVector(v) * 2 - *this; }
    };
    /* #endregion */
...
#define Segment Line
    struct Line {
        ...
        friend ostream &operator<<(ostream &out, const Line &l) { return (out << setprecision(2) << fixed << "y = " << l.d.y / l.d.x << "x + " << l.s.y - l.d.y / l.d.x * l.s.x); }
        // 极角
        ...
        // 点是否在线段上
        bool IsOnSegment(const Point &P) const { return this->IsOnLine(P) and (P - s).dot(e - P) >= 0; }
        // 点是否落在半平面内
        bool IsInHalfPlane(const Point &P) const { return d.cross(P - s) > 0; }
        // 和另一条直线、线段的位置关系
        STATUS Relation(const Line &L, bool is_segment = false) const {
            if (!is_segment) return d.cross(L.d) != 0 ? STATUS::IntersectionLine : (intercept() == L.intercept() ? STATUS::SameLine : STATUS::Parallel);
            else {
                if (d.cross(L.d) == 0) {
                    if (intercept() != L.intercept()) return STATUS::Parallel;
                    else return (IsOnSegment(L.s) or IsOnSegment(L.e) ? STATUS::OnSameLineOverlap : STATUS::OnSameLineDisjoint);
                } else {
                    // 快速排斥实验
                    bool ok1 = max(s.x, e.x) >= min(L.s.x, L.e.x) and max(L.s.x, L.e.x) >= min(s.x, e.x) and max(s.y, e.y) >= min(L.s.y, L.e.y);
                    // 跨立实验
                    bool ok2 = 1;
                    ok2 = ok2 and (L.s - s).cross(e - s) * (e - s).cross(L.e - s) >= 0;
                    ok2 = ok2 and (s - L.s).cross(L.e - L.s) * (L.e - L.s).cross(e - L.s) >= 0;
        ...
        // 点到直线距离
        Decimal DistanceOfPoint(const Point &P, bool directed = false) const { return directed ? d.cross(P - s) : d.cross(P - s).abs(); }
        // 平行直线之间的距离
        Decimal DistanceBetweenLine(const Line &l) const { return PerpVector(l.s).Scale().abs(); }
        // 点到直线的垂直向量
        Vector PerpVector(const Point &P) const { return (P - s) - (P - s).ProjectVector(d); }
        // 过定点作直线的垂线
    ...
    struct Polygon : Shape {
        ...
        Decimal Area() const {
            Decimal ans = 0;
            for (int i = 0, n = int(_v.size()); i < n; i++) ans += _v[i].cross(_v[(i + 1) % n]) * 0.5;
            return ans;
        }
        // 给定点集，求凸包
        vector<Point> GetConvex(bool inplace = false) {
            ...
            for (int i = 0, n = _v.size(); i < n; i++) {
                while (top > 0 && check(_v[i])) top--, _stack.pop_back();
                _stack.push_back(_v[i]), top++;
            }
            int t = top;
            for (int n = _v.size(), i = n - 2; i >= 0; i--) {
                while (top > t && check(_v[i])) top--, _stack.pop_back();
                _stack.push_back(_v[i]), top++;
            }
            _stack.pop_back(), top--;
            if (inplace) swap(_v, _stack);
            return _stack;
        }
    };
    ...
    struct Circle : Shape {
        ...
        Point Coord(const Decimal &ang) const { return c + Point{r * ang.cos(), r * ang.sin()}; }

        // 给定两圆，求内外公切线
        // tuple[0]: 切线在圆 A 上的切点
        // tuple[1]: 在 B 上的切点
        // tuple[3]: 两圆的相对位置关系
        static tuple<vector<Point>, vector<Point>, CircleRelation> Tangent(Circle &A, Circle &B) {
            vector<Point> a, b;
        ...
        static Circle BoundCircle(const Point &a, const Point &b, const Point &c) {
            ...
            Circle ret;
            ret.c = {(b2 * c1 - b1 * c2) / (b2 * a1 * 2 - b1 * a2 * 2), (a2 * c1 - a1 * c2) / (a2 * b1 * 2 - a1 * b2 * 2)};
            ret.r = (ret.c - a).Scale();
            return ret;
        }
        // 随机化求最小圆覆盖，期望时间复杂度线性
        static Circle MinEnclosingCircle(vector<Point> &v) {
            ...
            ans.c = v[0], ans.r = 0;
            for (int i = 1; i < n; i++) {
                if (ans.isInside(v[i])) continue;
                ans.c = v[i], ans.r = 0;
                for (int j = 0; j < i; j++) {
                    if (ans.isInside(v[j])) continue;
                    ans.c = (v[i] + v[j]) / 2;
                    ans.r = (ans.c - v[j]).Scale();
                    for (int k = 0; k < j; k++) {
                        if (ans.isInside(v[k])) continue;
    ...
    struct HalfPlane {
        ...
        bool Solve() {
            ...
            for (int i = 1; i < n; i++) {
                ...
                if (q[t].d.cross(ln[i].d) == 0) q[t] = ln[i].IsInHalfPlane(q[t].s) ? q[t] : ln[i];
                else q[++t] = ln[i];
    ...
    struct Misc {
        ...
        static tuple<Point, Point, Decimal> NearestPair_Deter(vector<Point> &p) {
            ...
            for (int n = int(p.size()), l = 0, r = 0; r < n; ++r) {
                while (l < r && p[r].x - p[l].x >= D) q.erase(q.find(p[l++]));
    ...
    struct Inverse {
        ...
        static Circle invC2C(Point &o, Decimal &&r, Circle &a) {
            ...
            Decimal RB = r.sqr() * 0.5 * (Decimal(1) / (OA - a.r) - Decimal(1) / (OA + a.r));
